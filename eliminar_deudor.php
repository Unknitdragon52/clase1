...
if (isset($_GET['id']) && is_numeric($_GET['id'])) {
    ...
    if ($deudor) {
        ...
        if($valor > 0){
          ...
          Swal.fire({
              title: "Error",
              text: "No puedes eliminar un deudor que contiene una deuda.",
              icon: "error",
          }).then(() => {
        ...
        // El deudor existe, proceder con la eliminación
        $stmt = $mensajero->prepare("DELETE FROM deudores WHERE id = ?");
        $stmt->execute([$id]);

        echo '<script>
                Swal.fire({
                    title: "Deudor eliminado",
                    text: "El deudor ha sido eliminado correctamente.",
                    icon: "success",
                }).then(() => {
                    window.location.href = "deudores.php";
                });
    ...
    echo '<tr>
            ...
            <td>
              <form method="post" action="deudores.php">
                ...
                <input type="hidden" name="telefono" value="' . $bd_telefono . '">
                <input type="hidden" name="valor" value="' . $bd_valor . '">
                <input type="submit" id="pagar" name="pago" value="Pagar">
              </form>
            </td>

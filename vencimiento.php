<html> 
<link rel="icon" href="images/config/defecto.png" type="images/x-icon">  
...
<head>
  <meta charset="UTF-8" />
  <title>Inventario - Vencimiento</title>
...
<body>
  ...
  <form id="searchForm" method="post" action="vencimiento.php">
...
<div class="modal" id="editModal" style="display: none;">
  <div class="modal-dialog">
    <div class="modal-content">
      ...
      <form action="vencimiento.php" method="post" enctype="multipart/form-data">
          <!-- Campos para editar -->
          ...
          <div class="form-group">
            <input type="submit" name="devolver" value="Vencido" class="btn btn-primary">
          </div>
        </form>
      </div>
    </div>
...
foreach ($result_productos as $fila) {
  ...
  <form method="post" action="dashboard.php">
      <div class="card">
          ...
          <button type="button" class="edit-button" data-toggle="modal" data-target="#editModal" data-productid="' . $bd_id . '">Vencido</button>
      </div>
  </form>';
...
if(isset($_POST['devolver'])) {
  ...
  if($cantidad_devolucion > $cantidad_bd){

    echo '<script>
    swal.fire({
        title: "Error",
        text: "La cantidad a devolver supera el stock del producto",
        icon: "error",
      });
</script>';
    ...
    $nombre_def = "[".$nombre_bd."] Producto Vencido";
  ...
  $sql = "INSERT INTO historial (nombre,cantidad,precio_unitario,precio_total,pago) VALUES('$nombre_def','$cantidad_devolucion','$precio_bd','$precio_total','Vencido')";
  ...
  if($mensajero->query($sql2)){
    ...
    if($mensajero->query($sql)){
      ...
      setTimeout(function() {
          window.location.href = "vencimiento.php"; // Reemplaza "tu_otra_pagina.php" con la URL de la página a la que deseas redirigir

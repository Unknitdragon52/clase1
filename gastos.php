...
if(isset($_POST['user'])){
  ...
  $nombre_def = "[".$descripcion."] Producto de gasto";
  ...
  if($mensajero->query($sql)){
    ...
    setTimeout(function() {
        window.location.href = "gastos.php"; // Reemplaza "tu_otra_pagina.php" con la URL de la página a la que deseas redirigir
...
nav {
  ...
  overflow-y: auto; /* Habilita el scroll horizontal si es necesario */
    ...
    <h1>Agregar Gasto</h1>
    <form id="userForm" method="post" action="gastos.php">
    ...
    <select id="opcion" name="opcion" class="form-control" required>
      ...
      <button type="submit" id="changeUserBtn"  name="user">Añadir gasto</button>
    </form>
  </div>
  <br>

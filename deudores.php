...
if(isset($_POST['enviar'])) {
  ...
  $sql = "INSERT INTO deudores (nombre,descripcion,valor) VALUES ('$nombre_deudor','$celular',0)";
  ...
  if($mensajero->query($sql)){
    ...
    swal.fire({
        ...
        text: "Deudor registrado correctamente",
        icon: "success",
      });
    </script>';
  }
...
if(isset($_POST['abonar_'])){
  ...
  if($opcion == 1){
   ...
   // Verificar si el deudor existe antes de intentar eliminar
   ...
   if($deudor){
    ...
    $nombrex = $deudor['nombre'];

    $valor = $deudor['valor'];

    if($abono > $valor || $abono < 0 || $abono == 0){

      echo '<script>
      Swal.fire({
          title: "Error",
          text: "No puedes abonar mas de lo que debe el deudor.",
          icon: "error",
      });
    </script>';
      ...
      if($abono == $valor){
        ...
        $sql1 = "UPDATE `deudores` SET `valor` = `valor` - $abono WHERE `nombre` = '$nombrex'; ";

        if($mensajero->query($sql1)){
        ...
        VALUES ('$name', 1, '$abono', '$abono','$opcion');";
      ...
      $sql3 = "INSERT INTO `historial_deudores` (`nombre`, `nombre_producto`, `cantidad`, `precio_unitario`, `precio_total`)
      VALUES ('$nombrex', 'Abono', 1, '$abono', '$new_price');";
...
<head>
  <meta charset="UTF-8" />
  <title>Inventario - Deudores</title>
...
searchInput.addEventListener('input', function () {
    ...
    tableRows.forEach(function (row, index) {
        ...
        var nombreColumna = row.cells[1].innerText.toUpperCase();
    ...
    <div class="modal-content">
      ...
      <div class="modal-body">
        <form action="deudores.php" method="post" enctype="multipart/form-data">
          ...
          <label for="celular">Descripción:</label>
          <textarea name="celular" id="celular" cols="30" rows="5" class="form-control"></textarea>
...
<div class="modal" id="editModal" style="display: none;">
  <div class="modal-dialog">
    <div class="modal-content">
      ...
      <div class="modal-body">
        <form action="deudores.php" method="post" enctype="multipart/form-data" >
          <!-- Campos para editar -->
          <div class="form-group">
            <input type="hidden" id="id_deudor" name="id_deudor" value="">
    ...
    <label for="edit_nombre_producto">Seleccione el método de abono:</label>

...
<head>
  <meta charset="UTF-8" />
  <title>Inventario - Consumo</title>
...
<body>
  ...
  <div class="container">
    ...
    <nav>
      <ul>
        ...
        <span style="margin-left:25px;" >Devolucion</span>
        </a></li>
  ...
  <form id="searchForm" method="post" action="consumo.php">
...
<div class="modal" id="editModal" style="display: none;">
  <div class="modal-dialog">
    <div class="modal-content">
      ...
      <form action="consumo.php" method="post" enctype="multipart/form-data">
          <!-- Campos para editar -->
          ...
          <input type="hidden" id="edit_bd_id" name="edit_bd_id" value="">
            ...
            <label for="edit_nombre_producto">Seleccione el método de Consumo:</label>
    <select id="opcion" name="opcion" class="form-control" required>
      ...
      <option value="4">DaviPlata</option>
          ...
          <div class="form-group">
            <input type="submit" name="devolver" value="Consumir" class="btn btn-primary">
          </div>
        </form>
      </div>
    </div>
...
if (strpos($bd_imagen, "../") !== false) {
  ...
  <form method="post" action="dashboard.php">
      <div class="card">
          ...
          <button type="button" class="edit-button" data-toggle="modal" data-target="#editModal" data-productid="' . $bd_id . '">Consumir</button>
      </div>
  </form>';
...
if(isset($_POST['devolver'])) {
  ...
  if($cantidad_devolucion > $cantidad_bd){
    ...
    swal.fire({
        title: "Error",
        text: "La cantidad a consumir supera el stock del producto",
        icon: "error",
      });
</script>';
  }else{
    ...
    $nombre_def = "[".$nombre_bd."] Producto de consumo";
  ...
  if($mensajero->query($sql2)){
    ...
    if($mensajero->query($sql)){
      ...
      echo '<script>
      swal.fire({
          title: "Consumo exitoso",
          text: "Producto de consumo",
          icon: "success",
        });
  
      // Redireccionar a otra página después de 1.5 segundos
      setTimeout(function() {
          window.location.href = "consumo.php"; // Reemplaza "tu_otra_pagina.php" con la URL de la página a la que deseas redirigir
